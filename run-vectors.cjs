  ...
  GOVERNED.forEach(repo => {
    ...
    function walk(d) {
      fs.readdirSync(d, { withFileTypes: true }).forEach(entry => {
        ...
        const rel = path.relative(isCore ? process.cwd() : path.dirname(process.cwd()), full).split(path.sep).join("/");
